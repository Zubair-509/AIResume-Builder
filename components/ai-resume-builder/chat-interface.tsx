...
${messages.map(m => `${m.type === 'bot' ? 'AI' : 'User'}: ${m.content}`).join('\n\n')}
...
User: ${userMessage}`;
      ...
      if (data.candidates && data.candidates[0] && data.candidates[0].content) {
        ...
        // Extract just the AI's response, not the entire conversation
        const cleanResponse = aiResponse.replace(/^AI:\s*/, '');
      ...
      return "I'm having trouble connecting to my AI services. Let's continue with our standard questions. " + 
...
All templates are ATS-optimized to ensure your resume passes through applicant tracking systems. Would you like to see all templates or should I recommend the best one for your background?`, 
      ...
      if (currentStep < chatSteps.length) {
        ...
        // Try to use Gemini API for the next response
        let botResponse;
        try {
          ...
          if (currentStep < chatSteps.length - 1) {
            botResponse = chatSteps[currentStep + 1].question;
          } else {
            botResponse = "Excellent! I have all the required information. Would you like to add any additional information to your resume? This could include certifications, projects, languages, achievements, or any other relevant details.";
          }
        }
        ...
        if (currentStep < chatSteps.length - 1) {
          ...
          addMessage('bot', botResponse, chatSteps[nextStep].suggestions);
        } else {
      ...
      addMessage('bot', "I'm sorry, I encountered an error processing your request. Let's continue with our standard questions.");
    } finally {
      setIsLoading(false);
    }
  };
